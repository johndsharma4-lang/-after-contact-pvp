export function patchSolarLancerRuntime(html) {
  ...
  // Heavy sustained penetrator: a complete beam can break a fresh local shield section and
  // burn through an intact compartment, while still resolving visibly over ten ticks.
  patched = patched.replace(
    ...
    "solar_lancer:{name:'SOLAR LANCER',kind:'laser',aim:'straight',damage:14,armorDamage:18,penetration:.72,exposureScale:.30,color:0xffd65a,duration:5,tickInterval:.5,maxCompartments:3,description:'Projected precision aim • heavy sustained penetrating solar laser • burns visibly through up to three compartments'}"
  );
  ...
  patched = patched.replace('</head>', '<meta name="ac-solar-lancer-runtime" content="projected-aim heavy-sustained-penetration damage-14 armor-18 penetration-72">\n</head>');
  return patched;
}
