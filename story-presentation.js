...
(() => {
  ...
  style.textContent=`
    /* Preserve the original readable crawl speed. */
    ...
    /* Frames 1–14: show one isolated panel, as large as possible, with no neighbouring art. */
    ...
    /* Frame 15 is no longer presented as a comic card: it becomes the title/start screen. */
    #storyComic.lastFrame{background:#000!important}
    #storyComic.lastFrame #storyPanelCanvas{width:100vw!important;height:100vh!important;max-width:none!important;max-height:none!important;left:0!important;top:0!important;transform:none!important;border-radius:0!important;box-shadow:none!important;object-fit:cover!important;background:#000!important}
    #storyComic.lastFrame:after{content:'';position:absolute;inset:0;z-index:2;pointer-events:none;background:linear-gradient(180deg,rgba(0,0,0,.08) 0%,rgba(0,0,0,0) 46%,rgba(0,0,0,.16) 67%,rgba(0,0,0,.66) 100%)}
    #storyComic.lastFrame #storyPanelCount,#storyComic.lastFrame #storyFrameBtn{display:none!important}
    #storyComic.lastFrame #storyContinue{display:block!important;z-index:6!important;left:50%!important;bottom:max(18px,calc(env(safe-area-inset-bottom) + 10px))!important;min-width:min(310px,48vw)!important;height:50px!important;padding:0 30px!important;border:1px solid rgba(255,255,255,.76)!important;border-radius:999px!important;background:linear-gradient(180deg,rgba(12,20,30,.92),rgba(2,7,12,.94))!important;color:#fff!important;font-size:13px!important;font-weight:1000!important;letter-spacing:.22em!important;text-shadow:0 2px 10px #000!important;box-shadow:0 12px 38px rgba(0,0,0,.72),0 0 22px rgba(90,185,255,.13)!important;backdrop-filter:blur(7px)!important;-webkit-backdrop-filter:blur(7px)!important}
