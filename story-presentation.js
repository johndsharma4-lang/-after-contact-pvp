...
(() => {
  ...
  const ASSET_VERSION='20260824-10';
  const PANELS=[
    ...
    `/story-panels/panel-05-v3.webp?v=${ASSET_VERSION}`,
  ...
  style.textContent=`
    ...
    body.acGameplayMode #storyIntro,body.acGameplayMode #storyComic{display:none!important;opacity:0!important;pointer-events:none!important;visibility:hidden!important}
    ...
    @media (orientation:landscape) and (max-height:430px){#acStartTitle{font-size:min(7.4vw,17vh)}#acStartSubtitle{margin-top:2vh;font-size:min(2.15vw,5.2vh)}#acStartTap{margin-top:3vh;font-size:min(1.6vw,3.8vh)}#acStartDisclaimer{font-size:min(.72vw,1.7vh)}}
  ...
  function hardCloseStoryForGameplay(){
    ...
    customSequenceActive=false;storyArmed=false;pendingLandscapeStart=null;crawlWasPlaying=false;
    intro.classList.remove('show','playing');intro.setAttribute('aria-hidden','true');
    comic.classList.remove('show','lastFrame','acStartMenu');comic.style.pointerEvents='none';
  ...
  function allowStoryAgain(){document.body.classList.remove('acGameplayMode');comic.style.removeProperty('pointer-events');}
  function syncGlobalOrientation(){
    const portrait=isPortraitPhone();document.body.classList.toggle('acGlobalPortrait',portrait);
    if(!portrait&&pendingLandscapeStart&&originalStartStoryIntro){const p=pendingLandscapeStart;pendingLandscapeStart=null;originalStartStoryIntro.apply(p.thisArg,p.args);}
    if(!portrait&&crawlWasPlaying&&intro.classList.contains('show')){crawlWasPlaying=false;void intro.offsetWidth;requestAnimationFrame(()=>intro.classList.add('playing'));}
  ...
  if(originalStartStoryIntro){window.startStoryIntro=function(...args){allowStoryAgain();storyArmed=true;if(isPortraitPhone()){pendingLandscapeStart={thisArg:this,args};syncGlobalOrientation();return false;}pendingLandscapeStart=null;return originalStartStoryIntro.apply(this,args);};}

  function loadImage(src){
    if(imageCache.has(src))return Promise.resolve(imageCache.get(src));
    return new Promise((resolve,reject)=>{const img=new Image();img.decoding='async';img.onload=()=>{if(img.naturalWidth<2||img.naturalHeight<2){reject(new Error('invalid image'));return;}imageCache.set(src,img);resolve(img)};img.onerror=()=>reject(new Error('load failed'));img.src=src;});
  }
  ...
  async function renderPanel(index){
    allowStoryAgain();customSequenceActive=true;comic.classList.remove('acStartMenu','lastFrame');panelIndex=Math.max(0,Math.min(index,PANELS.length-1));
    if(continueButton)continueButton.style.setProperty('display','none','important');if(nextButton){nextButton.style.removeProperty('display');nextButton.textContent='NEXT';}
    const img=await loadImage(PANELS[panelIndex]);drawContained(img);
  }
  function beginApprovedComic(){if(isPortraitPhone()||!storyArmed)return;storyArmed=false;customSequenceActive=true;preloadPanels().finally(()=>renderPanel(0).catch(()=>{}));}
  ...
  function resetStoryPresentation(){customSequenceActive=false;storyArmed=false;comic.classList.remove('acStartMenu');if(continueButton)continueButton.style.removeProperty('display');}
  function handleNext(e){if(!customSequenceActive||comic.classList.contains('acStartMenu'))return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();if(panelIndex<PANELS.length-1)renderPanel(panelIndex+1).catch(()=>{});else showStartMenu();}
  function handleStartMenuTap(e){if(!comic.classList.contains('acStartMenu'))return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();comic.classList.remove('acStartMenu');customSequenceActive=false;storyArmed=false;if(continueButton){continueButton.style.removeProperty('display');continueButton.click();}}

  if(nextButton){nextButton.addEventListener('pointerup',handleNext,true);nextButton.addEventListener('click',e=>{if(e.detail===0)handleNext(e)},true);}
  startMenu.addEventListener('pointerup',handleStartMenuTap,true);startMenu.addEventListener('click',e=>{if(e.detail===0)handleStartMenuTap(e)},true);
  new MutationObserver(()=>{syncGlobalOrientation();if(intro.classList.contains('show')&&!document.body.classList.contains('acGameplayMode'))storyArmed=true;}).observe(intro,{attributes:true,attributeFilter:['class']});
  new MutationObserver(()=>{syncGlobalOrientation();if(comic.classList.contains('show')&&storyArmed&&!customSequenceActive&&!comic.classList.contains('acStartMenu')&&!document.body.classList.contains('acGameplayMode'))beginApprovedComic();}).observe(comic,{attributes:true,attributeFilter:['class']});
  if(mpOverlay){new MutationObserver(()=>{if(!mpOverlay.classList.contains('hidden'))resetStoryPresentation();}).observe(mpOverlay,{attributes:true,attributeFilter:['class']});}
  for(const button of [soloButton,hostButton,joinButton]){if(!button)continue;button.addEventListener('pointerdown',hardCloseStoryForGameplay,true);}
  if(replayButton){replayButton.addEventListener('pointerdown',allowStoryAgain,true);replayButton.addEventListener('click',allowStoryAgain,true);}
  ...
  preloadPanels();syncGlobalOrientation();
})();