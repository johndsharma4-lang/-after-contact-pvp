...
function makeBeam(firstEnd){return{start:endpoint(0),segments:[],end:{id:firstEnd.id,copy(v){this.id=v.id}},visualEnd:{id:firstEnd.id},visualFrom:{id:firstEnd.id,copy(v){this.id=v.id}},extensionProgress:1,hold:.1,activeStage:-1,activeHit:null,blocked:false}}
const weapon={damage:52,armorDamage:80};
...

{
  ...
  assert.deepEqual(path.map(h=>h.room.armor),[20,100,100],'stage one may only damage cabin one');
  resolveStage({},path,weapon,1,beam);
  assert.deepEqual(path.map(h=>h.room.armor),[20,42,100],'stage two must resolve separately at 72% power');
  resolveStage({},path,weapon,2,beam);
  assert.deepEqual(path.map(h=>h.room.armor),[20,42,60],'stage three must resolve separately at 50% power');
  assert.equal(calls.shield,1,'the shield envelope is crossed once at entry');
  assert.deepEqual(calls.structure,[80,58,40]);
  assert.equal(beam.segments.length,3,'each resolved cabin must create its own visible penetration segment');
  assert.equal(beam.extensionProgress,0,'each new cabin resets the visible extension animation');
}

{
  const survivor={roomIndex:0,hp:100,active:true,exposedBySolar:false,exposedByBreach:false};crew.splice(0,crew.length,survivor);const path=makePath([100]),beam=makeBeam(path[0].end);shieldMode='open';calls.conceal=0;calls.crew.length=0;
  resolveStage({},path,weapon,0,beam);
  assert.equal(path[0].room.breach,80);
  ...
  assert.equal(calls.crew[0],37,'first-cabin crew damage remains strong without becoming an automatic kill');
  crew.length=0;
}

{
  assert.ok(base.includes("ACTUAL_CHARACTER_ART_${String(type).toUpperCase()}"),'combat rigs must mount the official detailed character artwork');
  assert.ok(base.includes("w.sprite=model.userData.combatCharacterSprite||null"),'impact and death presentation must receive the real character-art sprite');
  for(const art of['/bombardier.webp','/earth-sniper.webp','/combat-controller.webp','/solar-lancer.webp','/sun-disk-gunner-hq.webp','/sunadier-hq.webp'])assert.ok(base.includes(art),`missing combat artwork route ${art}`);
}
...
console.log('PASS Solar Lancer preserves sniper aim, stages three separate heavy penetration traces, stops at shields, and exposes official character artwork');
