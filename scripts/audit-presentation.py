...
checks = {
    ...
    'canon authority exposes six deep framed modules': "frame.name='AURELIAN_CANON_BAY_FRAME_'" in canon_authority and 'for(let i=0;i<specs.length;i++)acCanonBayFrame' in canon_authority and 'AURELIAN_CANON_MAIN_SPINE' not in canon_authority,
    'canon stepped silhouette has solid jointed prow': 'AURELIAN_CANON_COCKPIT_CROWN' in canon_authority and 'AURELIAN_CANON_STARBOARD_COMMAND_ROOF' in canon_authority and 'AURELIAN_CANON_AFT_VENTRAL_KEEL' in canon_authority and 'AURELIAN_CANON_CANNON_CRADLE_KEEL' in canon_authority and 'AURELIAN_CANON_PROW_CORE' in canon_authority and 'AURELIAN_CANON_PROW_JOINT_CORE' in canon_authority,
    'canon uses reference cockpit hub cannon and exhaust architecture': all(name in canon_authority for name in ['USER_CANON_IMG_3444','AURELIAN_COCKPIT_GLAZING','AURELIAN_CANON_CENTRAL_HUB_RING','AURELIAN_CANNON_CHAMBER_FACE','AURELIAN_CANON_EXHAUST_CLUSTER']),
    'canon damage exposes bays without replacing cutaway ownership': 'skin.userData.acDamagePresentationParts||[]' in base and 'const visualLoss=Math.min(1,ratio<=.50?ratio:.50+(ratio-.50)*2)' in base and 'panel.userData.structureHpHidden=removed' in base and 'fixedFrame=PRESERVED' in base,
    'progressive damage can remove every nonpersistent exterior part': 'removeCount=Math.min(count,Math.floor(visualLoss*count+.0001))' in base and 'wreckShellMinimum' not in base,
    'destroyed bay remains open through ordinary room sync': 'module.visible=!module.userData.structureHpHidden&&!room.erased&&breach<100' in base and 'if(removed)ensureStructureLossVoid(room);else clearStructureLossVoid(room)' in base,
    'structure-exposed crew become visible and directly targetable': 'function crewDirectlyTargetable(w,room)' in base and 'room.structureExposed' in base and 'crewDirectlyTargetable(occ,room)' in base and 'crewDirectlyTargetable(w,room)' in base,
    ...
    'aurelian closure restores only surviving exact module objects': 'visual.exteriorPanel.visible=!open&&!visual.exteriorPanel.userData?.structureHpHidden' in base and 'exteriorAuthority=UNIFIED_PRESSURE_HULL_EXACT_MODULES' in director and 'exactExteriorModules=' in director,
    ...
    'release muzzle is committed through final beam construction': 'acCommittedShotOriginWorld=flashPos.clone()' in weapon_origin and 'acCommittedShotWarrior===w&&acCommittedShotOriginWorld' in weapon_origin and 'acCommittedShotWarrior===attacker&&acCommittedShotOriginWorld' in weapon_origin and 'SHOT ORIGIN COMMIT' in weapon_origin,
