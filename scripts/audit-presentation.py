...
checks = {
    ...
    'enemy becomes visible early in aim': 'enemyVisibleAtFullAim:Y' in director and "enemyVisible='+(progress>=.30?'Y':'N')" in aim_block and 'smoothstep(progress,.02,.30)' in aim_block,
    ...
    'locked shooter frames both ships before drag': 'if(xrayOpen&&xrayConfirmedShooter&&!aiming)' in director and 'camera.position.lerp(battlePos,alpha)' in director,
