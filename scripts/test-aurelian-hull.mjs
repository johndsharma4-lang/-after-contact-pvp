...
const production=buildContext(full),productionSkin=new THREE.Group();production.buildAurelianDirectorHull(productionSkin,0,material);productionSkin.updateMatrixWorld(true);
assert.equal(productionSkin.userData.acCutawayBayPanels.length,6);let productionSamples=0;
for(const s of productionSkin.userData.acCutawayBayLayout)for(let x=-s.w*.40;x<=s.w*.40;x+=.62)for(let y=-s.h*.36;y<=s.h*.36;y+=.53){if(!inside([x,y],s.points.map(p=>[p[0]*.82,p[1]*.82])))continue;ray.set(new THREE.Vector3(s.x+x,s.y+y,30),new THREE.Vector3(0,0,-1));const hits=ray.intersectObject(productionSkin.userData.acPrimaryHull,true).filter(h=>h.point.z>2.0);assert.equal(hits.length,0,'Production art blocks bay '+s.index+' at '+[x,y]+' by '+hits.map(h=>h.object.name));productionSamples++}
assert.ok(productionSkin.getObjectByName('AURELIAN_COCKPIT_ARMORED_SLED'));assert.ok(productionSkin.getObjectByName('AURELIAN_CANNON_ARMORED_CRADLE'));assert.equal(descendants(productionSkin).filter(o=>o.name==='AURELIAN_REFERENCE_ENGINE_FAIRING').length,3);ok(productionSamples+' production-art aperture rays clear the reference pass');
const productionNodes=descendants(productionSkin),heroLeaves=productionNodes.filter(o=>o.name.startsWith('AURELIAN_BREAKAWAY_ARMOR_LEAF_')),heroCowls=productionNodes.filter(o=>o.name==='AURELIAN_HERO_ENGINE_COWL_UPPER'||o.name==='AURELIAN_HERO_ENGINE_COWL_LOWER'),heroCores=productionNodes.filter(o=>o.name==='AURELIAN_HERO_ENGINE_CORE');
assert.equal(heroLeaves.length,20);assert.equal(heroCowls.length,6);assert.equal(heroCores.length,3);assert.ok(productionSkin.getObjectByName('AURELIAN_HERO_DORSAL_CROWN_A'));assert.ok(productionSkin.getObjectByName('AURELIAN_HERO_VENTRAL_KEEL_B'));assert.ok(productionSkin.getObjectByName('AURELIAN_HERO_PROW_CAP'));ok('Hero pass builds twenty door-owned armor leaves, seven silhouette masses and deep triple-engine cowls');
const heroBounds=new THREE.Box3().setFromObject(productionSkin);assert.ok(heroBounds.max.x>26&&heroBounds.max.y>12&&heroBounds.min.y<-11);ok('Hero hull has the extended prow, crown and ventral keel silhouette');
const volumeNodes=descendants(productionSkin);assert.equal(volumeNodes.filter(o=>o.name==='AURELIAN_VOLUME_ENGINE_BARREL').length,3);assert.equal(volumeNodes.filter(o=>o.name==='AURELIAN_VOLUME_ENGINE_INNER_RING').length,9);assert.ok(productionSkin.getObjectByName('AURELIAN_VOLUME_COMMAND_BLISTER'));assert.ok(productionSkin.getObjectByName('AURELIAN_VOLUME_PROW_SPEAR'));assert.ok(productionSkin.getObjectByName('AURELIAN_VOLUME_CANNON_KEEL'));ok('Volume overhaul adds raised canopy, nested engines, deep prow and integrated cannon keel');
