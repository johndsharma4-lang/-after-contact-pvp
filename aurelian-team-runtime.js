export function patchAurelianTeamRuntime(html) {
  ...
  patched = patched.replace(
    ...
    "spatial_disintegrator:'/spatial-disintegrator.webp',sun_disk_gunner:'/sun-disk-gunner-hq.webp',sunadier:'/sunadier-hq.webp'"
  );

  patched = patched.replace(
    "function starterArt(type){return OFFICIAL_STARTER_ART[type]||starter3DArtCache[type]||null}",
    "function starterArt(type){const aurelianArt={sun_disk_gunner:'/sun-disk-gunner-hq.webp',sunadier:'/sunadier-hq.webp'};return aurelianArt[type]||OFFICIAL_STARTER_ART[type]||starter3DArtCache[type]||null}"
  );
