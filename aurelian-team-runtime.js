export function patchAurelianTeamRuntime(html) {
  ...
  patched = patched.replaceAll("Drag your SOLAR LANCER into any vessel compartment. You can reposition before battle.", "Deploy all three AURELIAN warriors into separate vessel compartments. You can reposition them before battle.");
  ...
  patched = patched.replace("diag('STARTER ART','transparent Acid Brute + Spatial Disintegrator cutouts embedded');", "diag('STARTER ART','Earth + Aurelian Level-2 roster art');");

  patched = patched.replace(/MATCH RECORDER v0\.33\.\d+/g, 'MATCH RECORDER v0.33.38');
  patched = patched.replace(/build=2026-08-(28|29)_[A-Z0-9_]+/g, 'build=2026-08-29_AURELIAN_ASSET_ROUTING_FIX');
  patched = patched.replace('</head>', '<meta name="ac-aurelian-team-runtime" content="three-warrior-deployment distinct-art unique-material static-assets sun-disk-gunner sunadier">\n</head>');
  return patched;
}
