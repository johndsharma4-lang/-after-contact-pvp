...
function spawnImpactCompartmentReveal(attacker,hit,duration=1450){if(!hit?.room)return;const room=hit.room,objects=[],targetCrew=opposing(attacker),hidden=[];for(const other of targetCrew){if(!other.active||other.roomIndex===hit.roomIndex)continue;for(const key of['sprite','xrayGlow','healthBase','healthFill']){const obj=other[key];if(obj?.visible){hidden.push(obj);obj.visible=false}}}const cloneWorld=o=>{if(!o)return null;const c=o.clone();const mats=Array.isArray(c.material)?c.material:[c.material];c.material=mats.map(m=>{const q=m.clone();q.transparent=true;q.opacity=.66;q.depthTest=false;q.depthWrite=false;return q});if(c.material.length===1)c.material=c.material[0];c.position.copy(o.getWorldPosition(new THREE.Vector3()));c.quaternion.copy(o.getWorldQuaternion(new THREE.Quaternion()));c.scale.copy(o.getWorldScale(new THREE.Vector3()));c.renderOrder=111;scene.add(c);objects.push(c);return c};const cavity=cloneWorld(room.cavity),edge=cloneWorld(room.edge),frame=cloneWorld(room.frame);if(cavity?.material)cavity.material.color.setHex(0x101820);if(edge?.material)edge.material.color.setHex(0xffd36a);if(frame?.material)frame.material.opacity=.18;const occupant=targetCrew.find(w=>w.active&&w.hp>0&&w.roomIndex===hit.roomIndex);if(occupant?.sprite?.material?.map){occupant.impactRevealUntil=performance.now()+duration;const s=new THREE.Sprite(new THREE.SpriteMaterial({map:occupant.sprite.material.map,color:0xffffff,transparent:true,opacity:.94,depthTest:false,depthWrite:false})),baseScale=new THREE.Vector3(occupant.passive?1.16:1.72,occupant.passive?2.02:3.02,1),base=warriorWorld(occupant).add(new THREE.Vector3(0,0,.58)),born=performance.now();s.position.copy(base);s.scale.copy(baseScale);s.renderOrder=114;scene.add(s);objects.push(s);(function react(now){if(!s.parent)return;const t=Math.min(1,(now-born)/duration),pulse=Math.sin(Math.min(1,t*2.8)*Math.PI);s.position.copy(base).add(new THREE.Vector3(Math.sin(t*24)*.12*pulse,-.12*pulse,0));s.scale.copy(baseScale).multiplyScalar(1+.055*pulse);s.material.color.setHex(t<.34?0xff6a3d:t<.58?0xffffff:0xffc0a2);if(t<1)requestAnimationFrame(react)})(born)}setTimeout(()=>{for(const obj of hidden)if(obj?.parent)obj.visible=true},duration+60);if(objects.length)effects.push({objects,life:duration/1000,max:duration/1000});diag('IMPACT COMPARTMENT REVEAL',\`side=\${structureTargetSide(attacker)} room=\${hit.roomIndex+1} occupant=\${occupant?'Y':'N'} isolated=Y\`)}
`;
