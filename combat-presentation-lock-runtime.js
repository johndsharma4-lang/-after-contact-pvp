...
`;
  ...
  patched = patched.replace(
    ...
    "const earthAlive=eWarriors.some(w=>w.active&&w.hp>0),aurelianAlive=aWarriors.some(w=>w.active&&w.hp>0);diag('VICTORY CREW CHECK',`earthPlayable=${earthAlive?'ALIVE':'DOWN'} aurelianPlayable=${aurelianAlive?'ALIVE':'DOWN'} passiveCrew=REMOVED`);"
  );
  patched = patched.replace(
...
let activeImpactCompartmentReveal=null,groupedHe9ImpactUntil=0;
...
function spawnImpactCompartmentReveal(attacker,hit,duration=1450){
 if(!hit?.room||!Number.isInteger(hit.roomIndex))return false;const room=hit.room,targetSide=structureTargetSide(attacker),targetCrew=opposing(attacker),occupant=targetCrew.find(w=>w.active&&w.hp>0&&w.roomIndex===hit.roomIndex)||targetCrew.find(w=>w.active&&w.roomIndex===hit.roomIndex)||null;if(!occupant){diag('IMPACT CUTAWAY SKIP',\`side=\${targetSide} room=\${hit.roomIndex+1} reason=NO_WARRIOR\`);return false}clearImpactCompartmentReveal();const hidden=[];
 ...
 if(occupant?.sprite?.material?.map){const dead=occupant.hp<=0,sprite=new THREE.Sprite(new THREE.SpriteMaterial({map:occupant.sprite.material.map,color:dead?0xa55b5b:0xffffff,transparent:true,opacity:dead?.62:1,depthTest:false,depthWrite:false}));sprite.position.set(0,-.18,.62);sprite.scale.set(2.55,3.78,1);sprite.userData.fixedDamageScale=sprite.scale.clone();sprite.renderOrder=118;group.add(sprite);occupant.impactRevealUntil=performance.now()+duration;occupant.impactRevealSprite=sprite;occupant.impactRevealGroup=group}const badgeCanvas=document.createElement('canvas');badgeCanvas.width=512;badgeCanvas.height=94;const bx=badgeCanvas.getContext('2d'),profile=STARTER_PROFILES[occupant.weaponKey],label=(profile?.name||'WARRIOR').toUpperCase();bx.fillStyle='rgba(4,9,14,.96)';bx.fillRect(3,3,506,88);bx.lineWidth=5;bx.strokeStyle=occupant.hp<=0?'#ff493d':'#79efa8';bx.strokeRect(3,3,506,88);bx.fillStyle=occupant.hp<=0?'#ff5b4d':'#ffffff';bx.font='900 '+(label.length>20?25:30)+'px system-ui';bx.textAlign='center';bx.textBaseline='middle';bx.fillText('ROOM '+(hit.roomIndex+1)+' • '+label,256,47);const badgeMap=new THREE.CanvasTexture(badgeCanvas);badgeMap.colorSpace=THREE.SRGBColorSpace;const badge=new THREE.Sprite(new THREE.SpriteMaterial({map:badgeMap,transparent:true,depthTest:false,depthWrite:false}));badge.position.set(0,2.16,.88);badge.scale.set(5.20,.95,1);badge.renderOrder=121;group.add(badge);
