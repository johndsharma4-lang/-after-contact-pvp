...
function spawnImpactCompartmentReveal(attacker,hit,duration=1450){
 ...
 const skin=targetSide==='aurelian'?factionSkinA:factionSkinE,module=skin?.userData?.damageModules?.[hit.roomIndex]||null,moduleWasVisible=!!module?.visible;if(module)module.visible=false;
 ...
 const metal=new THREE.MeshStandardMaterial({color:aurelianTarget?0x8c4f18:0x485761,metalness:.72,roughness:.34}),edgeMat=new THREE.MeshStandardMaterial({color:aurelianTarget?0xffc85a:0x9adfff,emissive:aurelianTarget?0xff7b18:0x318ac0,emissiveIntensity:.72,metalness:.62,roughness:.28}),inside=new THREE.MeshStandardMaterial({color:0x111820,metalness:.30,roughness:.76}),burn=new THREE.MeshStandardMaterial({color:0x24100a,emissive:0x5a1608,emissiveIntensity:.42,metalness:.18,roughness:.92});
 ...
 const occupant=targetCrew.find(w=>w.active&&w.hp>0&&w.roomIndex===hit.roomIndex)||null;if(occupant?.sprite?.material?.map){const sprite=new THREE.Sprite(new THREE.SpriteMaterial({map:occupant.sprite.material.map,color:0xffffff,transparent:true,opacity:1,depthTest:true,depthWrite:false}));sprite.position.set(0,-.28,.62);sprite.scale.set(occupant.passive?1.55:2.28,occupant.passive?2.56:3.62,1);sprite.userData.fixedDamageScale=sprite.scale.clone();sprite.renderOrder=114;group.add(sprite);occupant.impactRevealUntil=performance.now()+duration;occupant.impactRevealSprite=sprite;occupant.impactRevealGroup=group}
 ...
 const timer=setTimeout(()=>{if(activeImpactCompartmentReveal?.group===group)clearImpactCompartmentReveal()},duration+220);activeImpactCompartmentReveal={group,room,module,moduleWasVisible,hidden,occupant,timer};diag('IMPACT CUTAWAY',\`side=\${targetSide} room=\${hit.roomIndex+1} warrior=\${occupant?.weaponKey||'NONE'} oldXray=N fixedScale=Y\`)
}
`;
  ...
  patched = replaceExact(
    ...
    const focusRoot=impactFocusSide==='aurelian'?aure:earth,rootPoint=focusRoot.getWorldPosition(new THREE.Vector3()),focus=impactFocusPoint?rootPoint.clone().lerp(impactFocusPoint,.30):rootPoint;desiredPos=new THREE.Vector3(focus.x,focus.y+5.8,focus.z+62);desiredLook=new THREE.Vector3(focus.x,focus.y+.6,focus.z);desiredZoom=1.0;
  }else{
...
`;
  ...
  patched = patched.replace("sprite.scale.copy(baseScale).multiplyScalar(1+.08*Math.sin(Math.PI*t));","sprite.scale.copy(baseScale).multiplyScalar(1+.055*Math.sin(Math.PI*t));");
  ...
  status.cutaway=patched.includes('PRIVATE_PHYSICAL_CUTAWAY_LOCAL_ONLY');status.cutawayArt=patched.includes('xrayArtTextureForWarrior');status.earthCutaway=patched.includes("faction==='earth'")&&patched.includes('earthFortressCutawayTex');status.earthHullPanels=patched.includes('PHYSICAL CUTAWAY SHELL');status.physicalCutaway=patched.includes('cutawayPanel')&&!patched.includes('PRIVATE_XRAY_LOCAL_ONLY');status.impactReveal=patched.includes('function spawnImpactCompartmentReveal(')&&patched.includes('IMPACT CUTAWAY');
