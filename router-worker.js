...
function patchIndexHtml(html) {
  let patched = html;
  const patchStatus = {sniper:false, locator:false, support:false};
  ...
  patched = patched.replace(/MATCH RECORDER v0\.33\.\d+/g, 'MATCH RECORDER v0.33.19');
  patched = patched.replace(/build=2026-08-28_[A-Z0-9_]+/g, 'build=2026-08-28_EARTH_SPECIALISTS_VERIFIED');
  ...
  const statusText = `combatPatch=sniper:${patchStatus.sniper?'OK':'MISS'} controller:${patchStatus.locator?'OK':'MISS'} support:${patchStatus.support?'OK':'MISS'}`;
