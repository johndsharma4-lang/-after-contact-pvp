...
export function patchCombatPresentationDirectorRuntime(html) {
  if (html.includes('ac-presentation-director-v0341')) return html;

  let patched = html;
  const status = {director:false,singlePress:false,pressDrag:false,camera:false,turnSolo:false,turnMp:false,solar3dPreview:false,impact3d:false,scatterCamera:false,sunadierTrack:false,diskTrack:false,beamTrack:false,clearAim:false,turnVfxGate:false};

  const helpers = String.raw`
let acDirector={mode:'exterior',projectile:null,target:null,origin:null,hit:null,attacker:null,label:'',aimWindows:[],aimSignature:'',lastPreviewAt:0,settleUntil:0,travelTotal:1};
function acDirectorLocalTurn(){return multiplayer?currentTurn===localSide:soloTurn==='aurelian'}
function acDirectorRestoreAimWindows(){for(const state of acDirector.aimWindows||[]){if(state?.module)state.module.visible=state.moduleWasVisible&&!state.room?.erased&&!state.module.userData?.structureHpHidden;if(state?.group?.parent)state.group.parent.remove(state.group)}acDirector.aimWindows=[];acDirector.aimSignature=''}
function acDirectorReset(reason='reset'){acDirectorRestoreAimWindows();acDirector.mode='exterior';acDirector.projectile=null;acDirector.target=null;acDirector.origin=null;acDirector.hit=null;acDirector.attacker=null;acDirector.label='';acDirector.settleUntil=0;tacticalAimView=false;if(xrayOpen&&!acDirectorLocalTurn())closePrivateXray('director '+reason);diag('PRESENTATION DIRECTOR RESET',reason)}
...
function acDirectorBuild3DWindow(attacker,hit,order=0){
 ...
 const room=hit.room,targetSide=structureTargetSide(attacker),skin=targetSide==='aurelian'?factionSkinA:factionSkinE,module=skin?.userData?.damageModules?.[hit.roomIndex]||null,moduleWasVisible=!!module?.visible;if(module)module.visible=false;
 ...
 const aurelianTarget=factionForSide(targetSide)==='aurelian',metal=new THREE.MeshStandardMaterial({color:aurelianTarget?0x6d421f:0x344853,metalness:.5,roughness:.60,transparent:true,opacity:.98,depthWrite:true}),inside=new THREE.MeshStandardMaterial({color:0x050b11,metalness:.18,roughness:.92}),edgeColor=aurelianTarget?0xffc95c:0x93e5ff;
 const back=new THREE.Mesh(new THREE.BoxGeometry(5.65,3.88,.30),inside),floor=new THREE.Mesh(new THREE.BoxGeometry(5.65,.23,2.16),inside),ceiling=new THREE.Mesh(new THREE.BoxGeometry(5.65,.19,2.05),inside),leftWall=new THREE.Mesh(new THREE.BoxGeometry(.19,3.88,2.05),metal),rightWall=leftWall.clone();back.position.z=-.86;floor.position.set(0,-1.82,.05);ceiling.position.set(0,1.82,-.04);leftWall.position.set(-2.72,0,-.05);rightWall.position.set(2.72,0,-.05);group.add(back,floor,ceiling,leftWall,rightWall);
 const rim=new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.BoxGeometry(5.82,4.04,.58)),new THREE.LineBasicMaterial({color:edgeColor,transparent:true,opacity:.82,depthWrite:false}));rim.position.z=-.02;group.add(rim);
 const panels=[];for(const side of[-1,1]){const panel=new THREE.Mesh(new THREE.BoxGeometry(2.82,3.98,.26),metal.clone());panel.position.set(side*1.41,0,.25);panel.userData.closed=panel.position.clone();panel.userData.open=new THREE.Vector3(side*3.42,.10,.80);panel.userData.openRot=side*.82;group.add(panel);panels.push(panel)}
 const occupant=opposing(attacker).find(w=>w.active&&w.hp>0&&w.roomIndex===hit.roomIndex)||null;if(occupant){try{const rig=buildCutawayOnlyWarrior3D(occupant.weaponKey);if(rig){rig.position.set(0,-.54,.40);rig.scale.multiplyScalar(.68);rig.renderOrder=118;group.add(rig)}}catch(err){diag('3D WINDOW RIG FALLBACK',String(err?.message||err))}}
 const born=performance.now()+order*75;(function open(now){if(!group.parent)return;const t=Math.max(0,Math.min(1,(now-born)/390)),e=1-Math.pow(1-t,3);for(const panel of panels){panel.position.copy(panel.userData.closed).lerp(panel.userData.open,e);panel.rotation.y=panel.userData.openRot*e}if(t<1)requestAnimationFrame(open)})(performance.now());
 diag('3D TARGET WINDOW','room='+(hit.roomIndex+1)+' order='+(order+1)+' occupant='+(occupant?.weaponKey||'NONE')+' sequential=Y sprite=N');return{group,room,module,moduleWasVisible,roomIndex:hit.roomIndex}
}
function acDirectorPreviewSolarWindows(attacker,pt){if(!attacker||attacker.weaponKey!=='solar_lancer'||!aiming)return;const now=performance.now();if(now-acDirector.lastPreviewAt<90)return;acDirector.lastPreviewAt=now;const solution=solarBeamPathFromStage(attacker,pt,3),path=solution.path||[],signature=path.map(h=>h.roomIndex).join(',');if(signature===acDirector.aimSignature)return;acDirectorRestoreAimWindows();acDirector.aimSignature=signature;acDirector.aimWindows=path.map((hit,i)=>acDirectorBuild3DWindow(attacker,hit,i)).filter(Boolean);diag('SOLAR 3D AIM WINDOWS',signature?signature.split(',').map(i=>Number(i)+1).join('>'):'MISS')}
...
`;
  ...
  patched=replaceOnce(patched,"function renderEnemyAimOutlines(pointer){\n  if(!enemyAimOutlineLayer)return;enemyAimOutlineLayer.replaceChildren();if(!aiming||!selected)return;","function renderEnemyAimOutlines(pointer){\n  if(!enemyAimOutlineLayer)return;enemyAimOutlineLayer.replaceChildren();if(!aiming||!selected)return;if(selected.weaponKey==='solar_lancer')return;",status,'solar3dPreview');
  if(!patched.includes('acDirectorPreviewSolarWindows(selected,b);')){const next=patched.replace("function setAimVisual(a,b){\n  if(!selected)return;","function setAimVisual(a,b){\n  if(!selected)return;\n  acDirectorBeginAim(selected);acDirectorPreviewSolarWindows(selected,b);");status.solar3dPreview=status.solar3dPreview&&next!==patched;patched=next}
  ...
  if(xrayOpen&&aiming&&selected){
    const visual=xrayRoomVisuals?.find?.(v=>v.warrior===selected),shooter=visual?.rig3D?.getWorldPosition?.(new THREE.Vector3())||warriorWorld(selected),enemyRoot=selected.side==='aurelian'?earth:aure,enemy=enemyRoot.getWorldPosition(new THREE.Vector3()),center=shooter.clone().lerp(enemy,.50),span=Math.max(78,Math.abs(enemy.x-shooter.x)+48),vHalf=THREE.MathUtils.degToRad(camera.fov*.5),hHalf=Math.atan(Math.tan(vHalf)*camera.aspect),zNeed=(span*.5)/Math.max(.16,Math.tan(hHalf)),pointer=(typeof currentPx!=='undefined'&&currentPx)||{x:640,y:360},nx=Math.max(-1,Math.min(1,(pointer.x-640)/640)),ny=Math.max(-1,Math.min(1,(pointer.y-360)/360)),targetPos=new THREE.Vector3(center.x+nx*8,Math.max(shooter.y,enemy.y)+7-ny*2.2,Math.max(101,zNeed+27)),targetLook=new THREE.Vector3(center.x+nx*4.5,(shooter.y+enemy.y)*.5+1-ny*2.2,2),alpha=snap?1:.16;
    camera.position.lerp(targetPos,alpha);camera.zoom=THREE.MathUtils.lerp(camera.zoom,1.03,alpha);camera.updateProjectionMatrix();camera.lookAt(targetLook);return
  }
  ...
  if(occupant){try{const rig=buildCutawayOnlyWarrior3D(occupant.weaponKey);if(rig){rig.position.set(0,-.52,.40);rig.scale.multiplyScalar(.70);rig.renderOrder=118;group.add(rig);occupant.impactRevealGroup=group;occupant.impactRevealUntil=performance.now()+duration}}catch(err){diag('IMPACT 3D RIG ERROR',String(err?.message||err))}}
...
`;
  ...
  patched=patched.replace(/MATCH RECORDER v0\.33\.\d+/g,'MATCH RECORDER v0.34.1').replace(/MATCH RECORDER v0\.34\.0/g,'MATCH RECORDER v0.34.1');
  patched=patched.replace(/build=2026-09-(01|04)_[A-Z0-9_]+/g,'build=2026-09-04_REFINED_DIRECTOR_VFX_TURN_GATE_ENEMY_BIAS');
  const summary=Object.entries(status).map(([k,v])=>`${k}:${v?'OK':'MISS'}`).join(' ');
  return patched.replace('</head>',`<meta id="ac-presentation-director-v0341" name="ac-presentation-director" content="${summary}">\n</head>`);
}
