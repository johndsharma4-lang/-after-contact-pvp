...
function acDirectorReset(reason='reset'){acDirector.mode='exterior';acDirector.projectile=null;acDirector.target=null;acDirector.origin=null;acDirector.hit=null;acDirector.attacker=null;acDirector.label='';acDirector.settleUntil=0;acDirector.aimTargetProgress=0;acDirector.aimVisualProgress=0;acDirector.aimLastTs=0;acDirector.aimInputOrigin=null;acDirector.sealedRooms=0;acDirector.cameraStage=-1;tacticalAimView=false;if(typeof clearImpactFocus==='function')clearImpactFocus();if(xrayOpen&&!acDirectorLocalTurn())closePrivateXray('director '+reason);if(battleStarted&&!matchEnded)requestAnimationFrame(()=>updateBattleCamera(true));diag('PRESENTATION DIRECTOR RESET',reason+' camera=TWO_VESSEL_SNAP')}
...
`;
  ...
  if(xrayOpen&&xrayConfirmedShooter&&!aiming){
    ...
    // Lock the two-vessel frame as soon as a shooter is selected. A touch that
    // starts immediately after selection must see the same camera as release.
    camera.position.copy(battlePos);cameraLookTarget.copy(battleLook);camera.zoom=1;camera.aspect=16/9;camera.near=.1;camera.far=900;camera.updateProjectionMatrix();camera.lookAt(cameraLookTarget);return
  }
  if(xrayOpen&&aiming&&selected){
    const progress=acDirectorStepAimPresentation(performance.now()),sep=currentCombatRange(),maxY=Math.max(aure.position.y,earth.position.y),minY=Math.min(aure.position.y,earth.position.y),midX=(aure.position.x+earth.position.x)/2,midY=(maxY+minY)/2,rangeExtra=Math.max(0,Math.min(82,(sep-55)*.92)),altExtra=Math.max(0,(maxY-18)*.72),vHalf=THREE.MathUtils.degToRad(camera.fov*.5),hHalf=Math.atan(Math.tan(vHalf)*camera.aspect),horizontalNeed=(sep*.5+29)/Math.max(.18,Math.tan(hHalf)),verticalNeed=((maxY-minY)*.5+17)/Math.max(.18,Math.tan(vHalf)),battleZ=Math.max(92+rangeExtra+altExtra*.72,horizontalNeed,verticalNeed),battlePos=new THREE.Vector3(midX,27.5+midY*.11+altExtra*.22,battleZ),battleLook=new THREE.Vector3(midX,Math.max(10,midY*.48)+Math.min(8,sep*.045),0);
    // Screen-to-world aiming is only trustworthy when pointerdown, preview, and
    // release use one projection. Hull sealing may animate; the camera may not.
    camera.position.copy(battlePos);cameraLookTarget.copy(battleLook);camera.zoom=1;camera.aspect=16/9;camera.near=.1;camera.far=900;camera.updateProjectionMatrix();camera.lookAt(cameraLookTarget);
    if(acDirector.cameraStage!==1){acDirector.cameraStage=1;diag('AIM CAMERA LOCK','progress='+Math.round(progress*100)+'% framing=FIXED_TWO_VESSEL pointerProjection=STABLE')}
    return
  }
  ...
  if(acDirector.mode==='beam'&&acDirector.origin&&acDirector.target){
    const a=acDirector.origin,b=acDirector.target,center=a.clone().lerp(b,.50),span=Math.max(76,Math.abs(b.x-a.x)+58),vHalf=THREE.MathUtils.degToRad(camera.fov*.5),hHalf=Math.atan(Math.tan(vHalf)*camera.aspect),zNeed=(span*.5)/Math.max(.16,Math.tan(hHalf)),targetPos=new THREE.Vector3(center.x,Math.max(a.y,b.y)+6.5,Math.max(100,zNeed+27)),alpha=snap?1:.12;
    camera.position.lerp(targetPos,alpha);camera.zoom=THREE.MathUtils.lerp(camera.zoom,.98,alpha);camera.updateProjectionMatrix();camera.lookAt(center);return
  }`;
