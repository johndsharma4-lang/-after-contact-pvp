...
export function patchCombatPresentationDirectorRuntime(html) {
  if (html.includes('ac-presentation-director-v0400')) return html;

  let patched = html;
  const status = {director:false,singlePress:false,pressDrag:false,camera:false,turnSolo:false,turnMp:false,impact3d:false,scatterCamera:false,sunadierTrack:false,diskTrack:false,beamTrack:false,clearAim:false,turnVfxGate:false,preAimClosed:false};

  const helpers = String.raw`
let acDirector={mode:'exterior',projectile:null,target:null,origin:null,hit:null,attacker:null,label:'',settleUntil:0,travelTotal:1};
function acDirectorLocalTurn(){return multiplayer?currentTurn===localSide:soloTurn==='aurelian'}
function acDirectorReset(reason='reset'){acDirector.mode='exterior';acDirector.projectile=null;acDirector.target=null;acDirector.origin=null;acDirector.hit=null;acDirector.attacker=null;acDirector.label='';acDirector.settleUntil=0;tacticalAimView=false;if(xrayOpen&&!acDirectorLocalTurn())closePrivateXray('director '+reason);diag('PRESENTATION DIRECTOR RESET',reason)}
...
`;
  ...
  // Enemy hull stays closed while aiming. Only actual impact/breach presentation may reveal a room.
  ...
  if(!patched.includes('acDirectorBeginAim(selected);')){const next=patched.replace("function setAimVisual(a,b){\n  if(!selected)return;","function setAimVisual(a,b){\n  if(!selected)return;\n  acDirectorBeginAim(selected);");patched=next}
  ...
  if(xrayOpen&&aiming&&selected){
    const visual=xrayRoomVisuals?.find?.(v=>v.warrior===selected),shooter=visual?.rig3D?.getWorldPosition?.(new THREE.Vector3())||warriorWorld(selected),enemyRoot=selected.side==='aurelian'?earth:aure,enemy=enemyRoot.getWorldPosition(new THREE.Vector3()),center=shooter.clone().lerp(enemy,.50),span=Math.max(82,Math.abs(enemy.x-shooter.x)+54),vHalf=THREE.MathUtils.degToRad(camera.fov*.5),hHalf=Math.atan(Math.tan(vHalf)*camera.aspect),zNeed=(span*.5)/Math.max(.16,Math.tan(hHalf)),targetPos=new THREE.Vector3(center.x,Math.max(shooter.y,enemy.y)+7.5,Math.max(104,zNeed+28)),targetLook=new THREE.Vector3(center.x,(shooter.y+enemy.y)*.5+1,2),alpha=snap?1:.16;
    camera.position.lerp(targetPos,alpha);camera.zoom=THREE.MathUtils.lerp(camera.zoom,1.02,alpha);camera.updateProjectionMatrix();camera.lookAt(targetLook);return
  }
...
`;
  ...
  if(!patched.includes("function clearAim(){if(acDirector.mode==='aim')")){const next=patched.replace('function clearAim(){',"function clearAim(){if(acDirector.mode==='aim')acDirector.mode=xrayOpen?'cutaway':'exterior';tacticalAimView=false;");status.clearAim=next!==patched;patched=next}else status.clearAim=true;
  ...
  patched=patched.replace(/MATCH RECORDER v0\.3[34]\.\d+/g,'MATCH RECORDER v0.40.0');
  patched=patched.replace(/build=2026-09-(01|04|05)_[A-Z0-9_]+/g,'build=2026-09-05_STABLE_PRESENTATION_LIFECYCLE');
  const summary=Object.entries(status).map(([k,v])=>`${k}:${v?'OK':'MISS'}`).join(' ');
  return patched.replace('</head>',`<meta id="ac-presentation-director-v0400" name="ac-presentation-director" content="${summary} preAimEnemyHull:CLOSED aimCamera:STABLE impactReveal:POST_HIT_ONLY">\n</head>`);
}
