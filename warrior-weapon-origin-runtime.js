...
export function patchWarriorWeaponOriginRuntime(html) {
  if (html.includes('ac-warrior-visual-fire-v0414')) return html;
  let patched = html;
...
function acWarriorRelaxPose(w,delay=0){
 ...
 const joints=[body.pelvis,body.torso,body.headRoot,...(body.armRoots||[]),...(body.elbowRoots||[]),...(body.legRoots||[]),body.weapon,body.secondaryWeapon].filter(Boolean),startAt=performance.now()+Math.max(0,delay),duration=240;
 const settle=now=>{if(!rig.parent||rig.userData.acPoseToken!==token)return;if(now<startAt){requestAnimationFrame(settle);return}const alpha=Math.min(1,(now-startAt)/duration),ease=1-Math.pow(1-alpha,3);for(const joint of joints){const base=acAimJointBase(joint);joint.rotation.x=THREE.MathUtils.lerp(joint.rotation.x,base.rx,ease);joint.rotation.y=THREE.MathUtils.lerp(joint.rotation.y,base.ry,ease);joint.rotation.z=THREE.MathUtils.lerp(joint.rotation.z,base.rz,ease);joint.position.x=THREE.MathUtils.lerp(joint.position.x,base.px,ease);joint.position.y=THREE.MathUtils.lerp(joint.position.y,base.py,ease);joint.position.z=THREE.MathUtils.lerp(joint.position.z,base.pz,ease)}if(alpha<1)requestAnimationFrame(settle);else rig.userData.acAimPoseStage=null};requestAnimationFrame(settle)
}
function acWarriorPoseAim(w,stageTarget,fire=false){
 ...
 const pelvis=body.pelvis,torso=body.torso,head=body.headRoot,leftArm=body.armRoots?.[0],rightArm=body.armRoots?.[1]||body.armRoots?.[0],leftElbow=body.elbowRoots?.[0],rightElbow=body.elbowRoots?.[1],leftLeg=body.legRoots?.[0],rightLeg=body.legRoots?.[1],secondary=body.secondaryWeapon;
 const nodes=[pelvis,torso,head,leftArm,rightArm,leftElbow,rightElbow,leftLeg,rightLeg,weapon,secondary].filter(Boolean);for(const node of nodes)acAimJointBase(node);
 const key=w.weaponKey,axis=key==='solar_lancer'?Math.PI/2:key==='sun_disk_gunner'?-Math.PI/2:Math.atan2(-.52,1.30),axisName=key==='solar_lancer'?'LANCE_UP_AXIS':key==='sun_disk_gunner'?'GAUNTLET_FOREARM_AXIS':'CHAIN_GRENADE_AXIS',rigEuler=new THREE.Euler().setFromQuaternion(rig.getWorldQuaternion(new THREE.Quaternion()),'XYZ'),pelvisBase=acAimJointBase(pelvis),torsoBase=acAimJointBase(torso),weaponBase=acAimJointBase(weapon),rightArmBase=acAimJointBase(rightArm),rightElbowBase=acAimJointBase(rightElbow);
 const chainBase=rigEuler.z+(pelvisBase?.rz||0)+(torsoBase?.rz||0)+(weaponBase?.rz||0)+axis+((key==='sun_disk_gunner'||key==='sunadier')?((rightArmBase?.rz||0)+(rightElbowBase?.rz||0)):0),desired=-Math.atan2(dy,dx),delta=THREE.MathUtils.clamp(acAimAngle(desired-chainBase),-1.62,1.62),alpha=fire?.96:.62,facing=dx>=0?1:-1;
 ...
 else if(key==='sun_disk_gunner'){acAimJointZ(pelvis,-delta*.08,alpha);acAimJointZ(torso,delta*.16,alpha);acAimJointZ(rightArm,delta*.68,alpha);acAimJointZ(rightElbow,delta*.24,alpha);acAimJointZ(weapon,0,alpha);acAimJointZ(leftArm,delta*.62,alpha);acAimJointZ(leftElbow,delta*.22,alpha);acAimJointZ(secondary,0,alpha)}
 else{acAimJointZ(pelvis,-delta*.07,alpha);acAimJointZ(torso,delta*.16,alpha);acAimJointZ(rightArm,delta*.60,alpha);acAimJointZ(rightElbow,delta*.23,alpha);acAimJointZ(weapon,delta*.08,alpha);acAimJointZ(leftArm,delta*.28-facing*.16,alpha);acAimJointZ(leftElbow,delta*.10+facing*.14,alpha)}
 const pelvisPose=acAimJointBase(pelvis),torsoPose=acAimJointBase(torso),headPose=acAimJointBase(head),leftLegPose=acAimJointBase(leftLeg),rightLegPose=acAimJointBase(rightLeg);if(pelvisPose){pelvis.rotation.y=THREE.MathUtils.lerp(pelvis.rotation.y,pelvisPose.ry-facing*.05,alpha);pelvis.position.x=THREE.MathUtils.lerp(pelvis.position.x,pelvisPose.px-facing*.035,alpha)}if(torsoPose){torso.rotation.y=THREE.MathUtils.lerp(torso.rotation.y,torsoPose.ry+facing*.11,alpha);torso.position.x=THREE.MathUtils.lerp(torso.position.x,torsoPose.px+facing*.075,alpha);torso.position.y=THREE.MathUtils.lerp(torso.position.y,torsoPose.py+.025,alpha)}if(headPose){head.rotation.z=THREE.MathUtils.lerp(head.rotation.z,headPose.rz+delta*.24,alpha);head.rotation.y=THREE.MathUtils.lerp(head.rotation.y,headPose.ry+facing*.19,alpha)}if(leftLegPose){leftLeg.rotation.z=THREE.MathUtils.lerp(leftLeg.rotation.z,leftLegPose.rz-facing*.11,alpha);leftLeg.position.x=THREE.MathUtils.lerp(leftLeg.position.x,leftLegPose.px-facing*.045,alpha)}if(rightLegPose){rightLeg.rotation.z=THREE.MathUtils.lerp(rightLeg.rotation.z,rightLegPose.rz+facing*.07,alpha);rightLeg.position.x=THREE.MathUtils.lerp(rightLeg.position.x,rightLegPose.px+facing*.025,alpha)}
 const poseStage=Math.round(delta/.22);if(rig.userData.acAimPoseStage!==poseStage){rig.userData.acAimPoseStage=poseStage;diag('WARRIOR AIM BODY',key+' axis='+axisName+' chain=LEGS>HIPS>TORSO>HEAD>ARMS>WEAPON alignment=COHERENT delta='+THREE.MathUtils.radToDeg(delta).toFixed(0)+'deg muzzle=LIVE rawTarget=UNCHANGED')}
 if(!fire)return;if(torsoPose){torso.position.x-=facing*.13;torso.rotation.z-=facing*.045}if(pelvisPose)pelvis.position.x-=facing*.045;if(weaponBase){weapon.position.x-=facing*.08;weapon.position.y+=.055}const flashPos=muzzle.getWorldPosition(new THREE.Vector3());aimOriginWorld=flashPos.clone();aimOriginStage=worldToStage(aimOriginWorld);const flash=glowSphere(.42,WEAPONS[key]?.color||0xffd66b,12);flash.position.copy(flashPos);flash.material.transparent=true;flash.material.opacity=1;scene.add(flash);effects.push({objects:[flash],life:.24,max:.24});rig.userData.acPoseFiredUntil=performance.now()+175;diag('WARRIOR MUZZLE FIRE',key+' weaponRoot='+String(weapon.name||'UNNAMED')+' recoil=FULL_BODY muzzle=LIVE rawTarget=UNCHANGED')
}
...
`;
  ...
  return patched.replace('</head>','<meta id="ac-warrior-visual-fire-v0414" name="ac-warrior-visual-fire" content="'+summary+' aimPose:FULL_BODY_WEAPON_SPECIFIC poseDiagnostic:WARRIOR_AIM_BODY muzzle:LIVE_ARTICULATED rawTarget:UNCHANGED camera:NONE retry:NONE">\n</head>')
}
