...
function acAddArmAssembly(shoulders,mats,sx,style='standard'){
  ...
  const wrist=acAurelianMesh(new THREE.TorusGeometry(.18,.045,8,20),sun,'wristSeal'+q);wrist.rotation.x=Math.PI/2;wrist.position.set(0,-.56,.09);elbow.add(wrist);const hand=acAurelianGroup('hand'+q);hand.position.set(0,-.63,.12);elbow.add(hand);hand.add(acAurelianMesh(new THREE.BoxGeometry(.24,.26,.30),black,'handMesh'+q));
...
function acAddLegAssembly(pelvis,mats,sx){
  ...
  const boot=acAurelianMesh(new THREE.BoxGeometry(.31,.26,.54),gold,'boot'+q);boot.position.set(0,-.74,.17);knee.add(boot);const sole=acAurelianMesh(new THREE.BoxGeometry(.35,.08,.62),black2,'bootSole'+q);sole.position.set(0,-.88,.20);knee.add(sole);
  return {hip,knee}
}
...
function acBaseAurelian(name,kind='solar',heavy=false){
  const root=acAurelianGroup(name);const mats={
    black:acAurelianMat(0x080a0f,.38,.44),black2:acAurelianMat(0x171b24,.58,.30),white:acAurelianMat(0xf2eadb,.48,.23),gold:acAurelianMat(0xd69a2e,.88,.17,0x5b2b05,.16),sun:acAurelianMat(0xffbd32,.52,.11,0xff7300,2.7),hot:acAurelianMat(0xffffc2,.28,.08,0xffb000,3.8),cloth:new THREE.MeshStandardMaterial({color:0xd9c69f,metalness:.06,roughness:.76,side:THREE.DoubleSide})
  };
